...
struct AboutSUIV: View { // Help, Details
    ...
    private var scan_view_description = "The scanning screen can be used to start and stop recordings using the green button. Additional controls are provided to change between different perspectives and underlay visualizations. The flashlight can be activated using the button on the right to light up dim environments. Extended recordings can eventually fill up the main memory. The bar at the bottom of the scanning screen indicates the live main memory utilization."
